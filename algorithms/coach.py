...
class Coach():
    ...
    def __init__(self, game, nnet, args):
        self.game = game
        self.nnet = nnet
        self.pnet = copy.deepcopy(self.nnet) # the previous one as the competitor network
        self.args = args
    ...
    def learn(self):
        ...
        for i in range(1, self.args['numIters'] + 1):
            ...
            if len(self.trainExamplesHistory) > self.args['numItersForTrainExamplesHistory']:
                log.warning(f"Removing the oldest iteration's examples in trainExamples. len(trainExamplesHistory) = {len(self.trainExamplesHistory)}")
                self.trainExamplesHistory.pop(0)

            # backup history to a file, the examples were collected using the model from the previous iteration, so (i-1)  
